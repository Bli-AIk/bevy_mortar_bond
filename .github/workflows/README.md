# GitHub Workflows for bevy_mortar_bond
...
- 自动检出 `mortar` 仓库并复制到 `../../../mortar`，以提供 `mortar_compiler` 的本地路径版本
  - 代码格式检查 (`cargo fmt`)
  - Clippy代码静态分析
  - 构建和测试crate
  - 安全审计检查
...
- 自动检出 `mortar` 仓库并复制到 `../../../mortar`，确保 `mortar_compiler` 路径可用
  - 更新依赖版本
  - 自动创建PR如果有更新
...
4. **项目名称**: 将所有引用更新为`bevy_mortar_bond`
...
`bevy_mortar_bond` 依赖于两个本地路径：

- `../bevy_ecs_typewriter`
- `../../../mortar`

所有工作流会把 `Bli-AIk/bevy_ecs_typewriter` 检出到 `external/bevy_ecs_typewriter` 并复制到 `../bevy_ecs_typewriter`，同时把 `Bli-AIk/mortar` 检出至 `external/mortar` 并复制到 `../../../mortar`，以保持与主仓库一致的目录结构。
### Actions版本
